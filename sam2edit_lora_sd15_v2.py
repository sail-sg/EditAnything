...
from utils.controlnet import ControlNetModelInpaint
...
def obtain_generation_model(base_model_path, lora_model_path, controlnet_path, generation_only=False):
    if generation_only:
        controlnet = ControlNetModelInpaint.from_pretrained(
...
class EditAnythingLoraModel:
    ...
    def process(self, condition_model, source_image, enable_all_generate, mask_image, control_scale, enable_auto_prompt, prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta):
        ...
        with torch.no_grad():
            ...
            inpaint_image = inpaint_image.float().cuda()
            inpaint_image = torch.cat([inpaint_image for _ in range(num_samples)], dim=0)
            print(inpaint_image.shape, control.shape)
            control = torch.cat((control, inpaint_image), dim=1)
            ...
            # mask_image = cv2.resize(
            #     mask_image, (W, H), interpolation=cv2.INTER_LINEAR)
            # mask_tensor = torch.from_numpy(mask_image[:,:,0].copy()).view(1,1,H,W).float().cuda()
            # mask_tensor = mask_tensor.repeat(control.shape[0], 1,1,1)
            # mask_tensor/=255
            # print(mask_tensor.max(), mask_tensor.min())
            # control = torch.cat((control, mask_tensor), dim=1)
