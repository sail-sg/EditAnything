...
def create_demo(base_model_path="../chilloutmix_NiPrunedFp32Fix", lora_model_path="../40806/mix4"):
    ...
    def obtain_generation_model(controlnet_path, generation_only=False):
        if generation_only:
    ...
    pipe = obtain_generation_model(default_controlnet_path, generation_only=False)
    ...
    def process(condition_model, source_image, enable_all_generate, mask_image, control_scale, enable_auto_prompt, prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta):
        ...
        if mask_image is None:
            if enable_all_generate:
                ...
                pipe = obtain_generation_model(config_dict[condition_model], enable_all_generate)
            else:
                mask_image = source_image["mask"]
        print("To Use:", config_dict[condition_model],
              "Current:", default_controlnet_path)
        ...
        with torch.no_grad():
            ...
            prompt_embeds, negative_prompt_embeds = get_pipeline_embeds(
                pipe, postive_prompt, negative_prompt, "cuda")
        ...
        with gr.Row():
            with gr.Column():
                ...
                enable_auto_prompt = gr.Checkbox(
                    label='Auto generate text prompt from input image with BLIP2: Warning: Enable this may makes your prompt not working.', value=True)
